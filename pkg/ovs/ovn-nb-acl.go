...
func (c *OVNNbClient) GetACL(parent, direction, priority, match string, tier int, ignoreNotFound bool) (*ovnnb.ACL, error) {
	...
	if err := c.ovsDbClient.WhereCache(func(acl *ovnnb.ACL) bool {
		...
		klog.Error(err)
		return nil, fmt.Errorf("get acl with 'parent %s direction %s priority %s match %s tier %d': %w", parent, direction, priority, match, tier, err)
	}
	...
	if len(aclList) == 0 {
		...
		return nil, fmt.Errorf("not found acl with 'parent %s direction %s priority %s match %s tier %d'", parent, direction, priority, match, tier)
	}

	if len(aclList) > 1 {
		return nil, fmt.Errorf("more than one acl with same 'parent %s direction %s priority %s match %s tier %d'", parent, direction, priority, match, tier)
	}
